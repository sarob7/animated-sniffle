...
# Mock services and Pydantic schemas (unchanged)
# ... (include DataFetchService, HighBandValidationService, PurposeSOFArgs, etc.)
...
class AgenticTransactionAnalyzer:
    ...
    def _initialize_tools(self) -> List[StructuredTool]:
        ...
        return [
            ...
            # ... (other tools: verify_sender_identity, verify_receiver_identity, validate_salary_information, validate_business_profile)
            StructuredTool.from_function(
    ...
    def analyze_transaction(self, mtcn: str) -> Dict[str, Any]:
        ...
        # Retrieve relevant patterns from pattern_library
        patterns = getattr(self, 'pattern_library', {})
        customer_data = json.loads(self._fetch_customer_data_tool({"mtcn": mtcn}))
        relevant_patterns = []
        for pattern_key, pattern_data in patterns.items():
            analysis = pattern_data.get("analysis", {})
            reflection = pattern_data.get("reflection", {})
            if (analysis.get("mtcn") == mtcn or
                customer_data.get("amount", 0) > 5000 and "High amount" in reflection.get("critical_factors", []) or
                customer_data.get("sender_id") == analysis.get("sender_id") or
                "patterns_to_remember" in reflection):
        ...
        # Limit to top 5 recent patterns
        relevant_patterns = sorted(relevant_patterns, key=lambda x: x["timestamp"], reverse=True)[:5]
        patterns_str = json.dumps(relevant_patterns, indent=2) if relevant_patterns else "No relevant patterns found."
        
        # Log patterns used
        if relevant_patterns:
            logger.info(f"Applied patterns for MTCN {mtcn}: {patterns_str}")
        
        # Construct strategy with pattern-based guidance
        strategy = [
            f"1. Start by fetching basic customer data using fetch_customer_data with MTCN: {mtcn}",
            "2. Based on initial findings and insights from previous analyses, decide what additional information you need. Pay attention to patterns such as: {', '.join([p for pattern in relevant_patterns for p in pattern.get('patterns_to_remember', [])])}",
        ...
        try:
            ...
            agent_output = self.agent.invoke({"input": initial_prompt})
            logger.info(f"Agent output: {json.dumps(agent_output, indent=2)}")
            result = agent_output.get("output", "No output returned by agent")
...
# AdvancedAgenticTransactionAnalyzer (unchanged)
...
def main():
    ...
    # First run (empty patterns)
    result1 = analyzer.analyze_transaction_with_learning("MTCN123456")
    print("First Analysis Result:")
    print(json.dumps(result1, indent=2))
    
    # Second run (uses patterns)
    result2 = analyzer.analyze_transaction_with_learning("MTCN789012")
    print("\nSecond Analysis Result:")
    print(json.dumps(result2, indent=2))
    
    # Verify pattern_library and patterns used
    print("\nPattern Library:")
    print(json.dumps(analyzer.pattern_library, indent=2))
    print("\nPatterns Used in Last Analysis:")
    print(json.dumps(analyzer.analysis_history[-1].get("patterns_used", []), indent=2))
